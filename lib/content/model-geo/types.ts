...
/**
 * Локализованный контент одной страны (per country per locale). Прозаические
 * поля (introHtml, closingHtml, тексты offers/expectations/faq) могут содержать
 * inline-ссылки в markdown-виде `[текст](/путь)` — рендер парсит их через
 * RichText (конвенция контент-слоя вакансий).
 * 18.09.2026: из комментария убраны поля marketContext и paymentsNote — их
 * в типе нет и никогда не было в рендере; писать их в файлы стран бесполезно.
 */
export type ModelGeoContent = {
   ...
   * 4–6 строк: график / опыт / локация / выплаты / старт / верификация.
   * Значения без процентов и оговорок (витринная подача 25.07.2026).
   */
...
export type ModelGeoUi = {
   ...
   * выйти можно в любой момент». ⛔ Слова «договор/контракт» запрещены;
   * процентов, gross/net и калькулятора здесь нет (витринное правило 25.07).
   */
  antiFud: string;
};
