 ...
 * Vanilla three.js so it stays light; auto-rotates with a slight tilt, pauses when
...
export default function LogoMark3DInner({ onReady, spin = false, speed = 0.5, rotX = 0.16, rotY = -0.5 }: Props) {
  ...
  useEffect(() => {
    ...
    // Bail to the 2D fallback if WebGL is unavailable.
    try {
    ...
    camera.position.set(0, 0.25, 9.0);
    ...
    const key2 = new THREE.DirectionalLight(0xffffff, 1.4); key2.position.set(-3, 4, -7); scene.add(key2);
    const rimPink = new THREE.PointLight(0xff5bb5, 90, 40); rimPink.position.set(-6, 2.5, -2); scene.add(rimPink);
    ...
    plaqueGeo.center();
    const faceMat = track(new THREE.MeshPhysicalMaterial({ color: 0x0a0a0f, metalness: 0.45, roughness: 0.3, clearcoat: 1, clearcoatRoughness: 0.08, reflectivity: 0.6 }));
    const silver = track(new THREE.MeshPhysicalMaterial({ color: 0xeef0f4, metalness: 1.0, roughness: 0.18, clearcoat: 0.5, clearcoatRoughness: 0.1 }));
    group.add(new THREE.Mesh(plaqueGeo, [faceMat, silver]));
    ...
    const sGeo = track(new THREE.SphereGeometry(1.32, 96, 96));
    const mkSphere = (tex: THREE.Texture, emis: THREE.Color, x: number) => {
      const m = track(new THREE.MeshPhysicalMaterial({ map: tex, emissive: emis, emissiveIntensity: 0.14, metalness: 0.0, roughness: 0.13, clearcoat: 1, clearcoatRoughness: 0.05, ior: 1.45 }));
      const mesh = new THREE.Mesh(sGeo, m); mesh.position.set(x, 0, 0); group.add(mesh);
    };
    mkSphere(gradTex('#FF5BB5', '#A855F7'), new THREE.Color(0xff5bb5), -0.62);
    mkSphere(gradTex('#00D4FF', '#7C3AED'), new THREE.Color(0x00d4ff), 0.62);

    const TZ = 1.62, SC = 1.5;
    const textMat = track(new THREE.MeshPhysicalMaterial({ color: 0xe9ebef, metalness: 1.0, roughness: 0.2, clearcoat: 0.5, clearcoatRoughness: 0.1 }));
    const bw = (geo: THREE.BufferGeometry) => geo.boundingBox!.max.x - geo.boundingBox!.min.x;

    const buildWord = (font: Font, label: string) => {
      const size = 0.92 * SC;
      const P = { font, size, depth: 0.34 * SC, height: 0.34 * SC, curveSegments: 10, bevelEnabled: true, bevelThickness: 0.04 * SC, bevelSize: 0.035 * SC, bevelSegments: 4 } as ConstructorParameters<typeof TextGeometry>[1];
      ...
      const gapHead = bw(gA) >= 0 ? bw(gHead) - bw(gA) - bw(gB) : 0;
      ...
      for (const z of [TZ, -TZ]) {
        ...
        for (const it of layout) {
          const m = new THREE.Mesh(it.geo, textMat); m.position.set(it.x, -cy, 0); side.add(m);
        }
        side.position.z = z;
